...
public class BrandCategoryTool {
	...
	/**
	 * * Retrieves brand data based on the provided query.
	 * 
	 * @param query The query used to retrieve brand data.
	 * @return The response containing brand categories, or null if no data is
	 *         found.
	 */
	...
	public void mapBrandCategoryValue(BrandCategoriesResponse brandCategoryResponse) {
		...
		if (brandCategoryResponse != null && brandCategoryResponse.getBrandCategories() != null
			...
			if (brandCategoryResponse.getBrandType().equals(SearchConstants.LARGE_BRAND)) {
				setLargeBrandCategories(brandCategoryResponse);
			} else if (brandCategoryResponse.getBrandType().equals(SearchConstants.MEDIUM_BRAND)) {
				setMediumBrandCategories(brandCategoryResponse);
			}
		}
		log.debug("BrandCategoryTool:::mapBrandCategoryValue:::END:{}");
	}

	private void setMediumBrandCategories(BrandCategoriesResponse brandCategoryResponse) {
		...
		if (!cat.getTitle().equalsIgnoreCase(SearchConstants.GENDER_UNISEX)) {
			...
			for (Category category : catagories) {
				category.getSubCategories().stream().forEach(subCat -> {
					...
					String catName = brandName + SearchConstants.BRAND_WITH_SPACE + genderTitle + SearchConstants.SPACE
							+ subCat.getTitle();
					...
					if (null != subCat.getEmbedId() && rrConfiguration.isEnableWidenImage()) {
						...
						fieldImageFile.setUrl(
								subCat.getImageurl().replace(SearchConstants.SCENE7_IMAGE, SearchConstants.RRS_IMAGE));
					}
	...
	private void setLargeBrandCategories(BrandCategoriesResponse brandCategoryResponse) {
		String brandName = brandCategoryResponse.getBrandName();
				...
				for (Category category : catagories) {
					if (!category.getTitle().equalsIgnoreCase(SearchConstants.GENDER_UNISEX)) {
						...
						category.getSubCategories().stream().forEach(subCat -> {
							...
							if (!ignoreSubCatagory.contains(
									subCat.getTitle().replace(SearchConstants.SPACE, SearchConstants.UNDERSCORE))) {
								...
								fieldDesktop.setDescription(brandName + SearchConstants.SPACE + subCat.getTitle());
								fieldDesktop.setFile(largeBrandFile);
								if (null != subCat.getEmbedId() && rrConfiguration.isEnableWidenImage()) {
									largeBrandFile.setImageId(subCat.getEmbedId());
								} else if (null != subCat.getImageurl()) {
									largeBrandFile.setUrl(subCat.getImageurl().replace(SearchConstants.SCENE7_IMAGE,
								...
								if (catMap.containsKey(
										subCat.getTitle().replace(SearchConstants.SPACE, SearchConstants.UNDERSCORE))) {
									itemFields.setTitle(catMap
											.get(subCat.getTitle().replace(SearchConstants.SPACE,
													SearchConstants.UNDERSCORE))
											.replace(SearchConstants.UNDERSCORE, SearchConstants.SPACE));
								} else {
									itemFields.setTitle(subCat.getTitle());
								}
								String replaceItemFields;
								if (subCat.getCategoryUrl().contains(SearchConstants.HOKA_ONE)) {
									replaceItemFields = subCat.getCategoryUrl().replace(SearchConstants.HOKA_ONE,
											SearchConstants.HOKA);
								...
								if (((String) SearchConstants.APPAREL).equalsIgnoreCase(category.getTitle())) {
									if (apparelOrderMap != null
											&& apparelOrderMap.containsKey(itemFields.getTitle().toLowerCase()
								...
								if ((SearchConstants.SHOES).equalsIgnoreCase(category.getTitle())) {
									if (shoeOrderMap != null && shoeOrderMap.containsKey(subCat.getTitle().toLowerCase()
											.replace(SearchConstants.SPACE, SearchConstants.UNDERSCORE))) {
